            ...
            int j0)              //!<
{
  ...
  for (i=0;i<BLOCK_SIZE;i++)
  {
    ...
    for (j=0;j<2;j++)
    {
	  ...
	  img->InitializingM7[i][j] = (m6[j] + m6[j1] + (img->mpr[i + ioff][j + joff] << DQ_BITS) + DQ_ROUND) >> DQ_BITS;
	  img->InitializingM7[i][j] = (m6[j] - m6[j1] + (img->mpr[i + ioff][j1 + joff] << DQ_BITS) + DQ_ROUND) >> DQ_BITS;
